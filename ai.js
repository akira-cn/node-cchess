...
var Evaluater = require('Evaluater').Evaluater;
...
AI = {
    ...
    // 初始化
    init : function(board, max_depth) {
        if(board)  Board = board;
        else Board.init_board();
        this.max_depth = max_depth || 3;
        this._init_hash_key();
    },
    ...
    play_a_chess : function(camp) {
        ...
        // 移动棋子
        if (Board.move_chess(m.fx, m.fy, m.tx, m.ty, {move_action : true})) {

            // 切换阵营
            this.cur_camp = -this.cur_camp;

            // 移动一步
            this.events.emit('step', m);
        }
